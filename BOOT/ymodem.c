...
/*******************************************************************************
  ...
  * @输入参数   buf: 首地址指针    FILE_SIZE_LENGTH=16
  * @输出参数   无
  * @返回参数   文件长度
*******************************************************************************/
int32_t Ymodem_Receive (uint8_t *buf)
{
    ...
    for (session_done = 0, errors = 0, session_begin = 0; ;)
    {
        ...
        {
            switch (Receive_Packet(packet_data, &packet_length, NAK_TIMEOUT))//接收来自电脑的一包数据，128或者1024数据（不算字头和字尾）
            {
            	case 0:errors = 0;
                    ...
                    else//传下来的正常的数据包
                    {
                        if (packets_received == 0)
                        {
                            ...
                            if (packet_data[PACKET_HEADER] != 0)
                            {
                                ...
                                //计算需要擦除Flash的页
																...
																user_flash_sector_erase( );//单纯的擦除一整sector
                                Send_Byte(ACK);
                                Send_Byte(CRC16);
                            }
                            //文件名数据包空，结束传输
                        ...
                        else
                        {
                            ...
                            RamSource = (uint32_t)buf;
														...
														HAL_FLASH_Unlock();														
//                            for (j = 0; (j < packet_length) && (FlashDestination <(APP_FLASH_ADDR + size)); j += 4)
//                            {
//                                //把接收到的数据编写到Flash中,只写不擦
//																uint8 num_out=0;
//																for(num_out=0;num_out<=10;num_out++)
//																{
//																	user_flash_word_wr(FlashDestination, *(uint32_t*)RamSource);	
...
//                                RamSource += 4;
//                            }
                            for (j = 0; (j < packet_length) && (FlashDestination <(APP_FLASH_ADDR + size)); j += 1)
                            {
                                //把接收到的数据编写到Flash中,只写不擦
//																uint8 num_out=0;
//																for(num_out=0;num_out<=10;num_out++)
...
//																for( ; ; )
//																{
																		user_flash_byte_wr(FlashDestination, *(uint8*)RamSource);	
//																		if (*(uint8*)FlashDestination != *(uint8*)RamSource)
//																		{
//																				user_flash_byte_wr(FlashDestination, 0xff);
//																		}
//																		else
//																		{
//																				break;
